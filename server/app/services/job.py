...
class JobService:
    ...
    async def set_succeeded(self, job_id: str, result_path: str, result_file_name: str, result_text: str | None = None, ocr_segments: list | None = None) -> Result[dict]:
        """
        标记任务成功

        Args:
            job_id: 任务 ID
            result_path: 结果文件实际路径
    ...
    async def _process_job(self, job_id: str) -> None:
        ...
        if not file_path and not job.get("params", {}).get("text"):
            await self.set_failed(job_id, "文件路径不存在，请先完成上传")
            return
        ...
        try:
            ...
            result: EngineResult = await engine.execute(file_path, job["params"])
            ...
            if result.success:
                if result.output_files:
                    ...
                    await self.set_succeeded(job_id, result_path, result_file_name, result_text, ocr_segments)
                else:
