...
/** 字段校验错误 */
export interface CronFieldErrors {
  minute?: string
  hour?: string
  dom?: string
  month?: string
...
/** 验证单个字段值在有效范围内 */
function validateField(token: string, def: FieldDef): string | undefined {
  // 通配符（纯 * 或带步长的 */N 如 */15）
  ...
  if (wildcardStepMatch) {
    ...
    if (stepVal >= 1) return undefined
  }

  // 步长格式 N/M（如 1-10/2 或 2/3）
  const stepParts = token.split('/')
  if (stepParts.length > 2) return `${token}：步长格式错误`
  ...
  if (rangeParts.length > 2) return `${token}：范围格式错误`

  if (rangeParts.length === 2) {
    const start = Number(rangeParts[0])
    const end = Number(rangeParts[1])
    if (isNaN(start) || isNaN(end)) return `${token}：范围值必须为数字`
    if (start < def.min || start > def.max) return `${token}：起始值 ${start} 超出范围 ${def.min}-${def.max}`
    if (end < def.min || end > def.max) return `${token}：结束值 ${end} 超出范围 ${def.min}-${def.max}`
    if (start > end) return `${token}：起始值不能大于结束值`
    return undefined
  }
  ...
  for (const part of listParts) {
    ...
    if (isNaN(num)) return `${token}：包含非数字值 "${part}"`
    if (num < def.min || num > def.max) return `${token}：值 ${num} 超出范围 ${def.min}-${def.max}`
  }
 ...
 * 校验 5 字段 Cron 表达式
 */
export function validateCron(expr: string): CronResult {
  const trimmed = expr.trim()
  if (!trimmed) return { valid: false, error: '表达式不能为空' }
  ...
  const fieldErrors: CronFieldErrors = {}
  let hasError = false
...
export function describeCron(expr: string): string {
  ...
  if (expr === '0 12 * * *') return '每天 12:00 执行'
