...
export function useFileUpload(maxConcurrent = 3) {
  ...
  const activeCount = computed<number>(() =>
    uploadStates.value.filter((s) => s.status === 'uploading').length
  )

  /** 是否有上传进行中 */
  const isUploading = computed<boolean>(() => activeCount.value > 0)
  ...
  async function uploadChunk(
    ...
    const [, err] = await to(
      apiClient.put(`/api/v1/uploads/${uploadId}/chunks/${chunkIndex}`, formData, { signal })
    )
    if (err) throw err
  }
  ...
  async function uploadFile(file: File): Promise<string | null> {
    ...
    const [createResult, createErr] = await to(
      apiClient.post<UploadCreateResponse>('/api/v1/uploads', {
        fileName: file.name,
        fileSize: file.size,
        mimeType: file.type,
    ...
    const uploadId: string = createResult.data.uploadId
    ...
    // 3. 标记上传完成
    const [, completeErr] = await to(
      apiClient.post(`/api/v1/uploads/${uploadId}/complete`)
    )
    if (completeErr) {
  ...
  function cancelAll(): void {
    ...
    uploadStates.value = uploadStates.value.map((s) =>
      s.status === 'uploading' ? { ...s, status: 'canceled' as const } : s
    )
  }
   ...
   * 更新上传状态（不可变模式）
   */
  function updateState(fileId: string, update: Partial<UploadState>): void {
    uploadStates.value = uploadStates.value.map((s) =>
      s.fileId === fileId ? { ...s, ...update } : s
    )
  }
